...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (IsCollision(s1, s2)) {
			...
			DrawSphere(s2.center, s2.radius, viewProjectionMatrix, viewportMatrix, 0xFF0000FF);
		} else {
			DrawSphere(s1.center, s1.radius, viewProjectionMatrix, viewportMatrix, 0xFFFFFFFF);
			DrawSphere(s2.center, s2.radius, viewProjectionMatrix, viewportMatrix, 0xFFFFFFFF);
		}
...
bool IsCollision(const Sphere& s1, const Sphere& s2)
{
	float distance = GetLength(s2.center) - GetLength(s1.center);
